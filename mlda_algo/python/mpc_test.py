...
# ===
# Convert from x,y,theta into the standard coordinates
...
class NMPCController():
    ...
    def main_NMPC(self,step,step_tot,x_ref,y_ref,theta_ref,vr_ref,vl_ref,ar_ref,al_ref,X0,occupancy_message,x_obstacles_all,y_obstacles_all,position_pub):
        ...
        if i_coords != []:
            x_obstacles = map_origin_x + (a_min + j_coords) * map_resolution # Get the coordinate from the map
            ...
            obstacle_constraints = ca.MX.zeros((self.N*len(voxel_midpoints),1)) ## Optimization variables number of obs blk x N
            k = 0
        ...
        multiple_constraints = 8 # constraints between pos, vel, acc per time step
        single_constraints = 5 # initial condition
        l_obs = obstacle_constraints.size()[0]
        self.lbg = np.zeros((self.N-1)*multiple_constraints + single_constraints + l_obs) # Equality constraints
...
def controller_node(x_traj, y_traj, theta_traj, t_traj, vr_traj, vl_traj, ar_traj, al_traj):
    ...
    listener = tf2_ros.TransformListener(tfBuffer) # Get the current state
    ...
    t_ref = np.arange(t_traj[0], t_traj[-1], h) # Number of time step
    x_ref = interp_x(t_ref) # interpolate based on the number of
    ...
    target_traj_msg = trajectory_plot(x_ref, y_ref, theta_ref) # Give goal position and heading
    cycle_times = []
    solver_times = []
    data_times = []
    marker_id = 0
