...
class LaserScanToPointCloud:
    def __init__(self):
        ...
        self.sub_laser_scan = rospy.Subscriber(
            self.TOPIC_LASER_SCAN, LaserScan, self.callback_laser_scan
        )
        self.pub_point_cloud = rospy.Publisher(
            self.TOPIC_POINT_CLOUD_LASER, PointCloud2, queue_size=1
        )

        self.pub_point = rospy.Publisher(
            self.TOPIC_POINT_CLOUD_ODOM, PointCloud2, queue_size=1
        )
