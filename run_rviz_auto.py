...
if __name__ == "__main__":
    ...
    gazebo_process = subprocess.Popen(
        ["roslaunch", launch_file, "world_name:=" + world_name, "gui:=true"]
    )
    ...
    launch_file = join(
        base_path, "..", "jackal_helper/launch/move_base_mlda_rviz_auto.launch"
    )
    ...
    nav_metric = (
        int(success)
        * optimal_time
        / np.clip(actual_time, 2 * optimal_time, 8 * optimal_time)
    )
